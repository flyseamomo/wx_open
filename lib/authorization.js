 ...
 */
exports.getCode = function (appId, redirect_uri, scope) {
...
exports.getUserInfoByAccessToken = async function (openid) {
    let access_token = await this.redis.get(`userInfo_access_token${res.data.openid}`)
    if (access_token){
      let res = await axios.get(`https://api.weixin.qq.com/sns/userinfo?access_token=${accessToken}&openid=${openid}&lang=zh_CN`)
      return res.data
    }else return null
}
